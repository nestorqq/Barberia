...
const sendConfirmationToClient = async (clienteEmail, clienteNombre, datos) => {
  try {
    if (!transporter) {
      console.error('[EMAIL ERROR] Transporter not initialized for client confirmation');
      throw new Error('Email service not initialized');
    }

    if (!clienteEmail) {
      console.error('[EMAIL ERROR] Client email is empty');
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px;">
        <div style="text-align: center; margin-bottom: 24px;">
          <h1 style="color: #1a1a2e; margin: 0;"> Barbería</h1>
    ...
    console.log(`[EMAIL SUCCESS] Correo confirmación enviado a CLIENTE (${clienteEmail}) - Message ID: ${info.messageId}`);
    if (info.response && info.response.includes('250')) {
      console.log(`[EMAIL SUCCESS] Respuesta del servidor: ${info.response}`);
    }
  } catch (error) {
    console.error(`[EMAIL ERROR] Error enviando correo a cliente (${clienteEmail}):`, error.message);
    throw error;
  }
};
...
const sendConfirmationToBarbero = async (barberoEmail, barberoNombre, datos) => {
  try {
    if (!transporter) {
      console.error('[EMAIL ERROR] Transporter not initialized for barber confirmation');
      throw new Error('Email service not initialized');
    }

    if (!barberoEmail) {
      console.error('[EMAIL ERROR] Barber email is empty');
    ...
    console.log(`[EMAIL SUCCESS] Correo nueva cita enviado a BARBERO (${barberoEmail}) - Message ID: ${info.messageId}`);
    if (info.response && info.response.includes('250')) {
      console.log(`[EMAIL SUCCESS] Respuesta del servidor: ${info.response}`);
    }
  } catch (error) {
    console.error(`[EMAIL ERROR] Error enviando correo a barbero (${barberoEmail}):`, error.message);
    throw error;
  }
};
...
const sendRefundToClient = async (clienteEmail, clienteNombre, datos) => {
  try {
    if (!transporter) {
      console.error('[EMAIL ERROR] Transporter not initialized for refund email');
      throw new Error('Email service not initialized');
    }

    if (!clienteEmail) {
      console.error('[EMAIL ERROR] Client email is empty for refund');
      throw new Error('Client email is required');
    }
    ...
    console.log(`[EMAIL SUCCESS] Correo reembolso enviado a CLIENTE (${clienteEmail}) - Message ID: ${info.messageId}`);
    if (info.response && info.response.includes('250')) {
      console.log(`[EMAIL SUCCESS] Respuesta del servidor: ${info.response}`);
    }
  } catch (error) {
    console.error(`[EMAIL ERROR] Error enviando correo de reembolso a cliente (${clienteEmail}):`, error.message);
    throw error;
  }
};
